 ...
 * 
 * FEATURES:
 * ✅ Syncs lesson-database sheet to Google Calendar
...
const CONFIG = {
  // Which sheets to sync
  SHEETS_TO_SYNC: ['lesson-database'],  // Main data sheet
  ...
  // Calendar settings
  CALENDAR_PREFIX: '📚 ',  // Prefix for event titles
  ...
  AUTO_SYNC_HOURS: 1,    // Sync every 1 hour
};
...
function syncSheet(ss, calendar, sheetName) {
  ...
  for (let i = 0; i < data.length; i++) {
    const row = data[i];
    const rowNum = i + 2;  // Actual row in sheet (starting from row 2)
...
function setupTriggers() {
  ...
  // Create time-based trigger (every hour)
  ...
  SpreadsheetApp.getUi().alert(
    '✅ Auto-Sync Enabled!\n\n' +
    `• Syncs every ${CONFIG.AUTO_SYNC_HOURS} hour automatically\n` +
...
function onEditTrigger(e) {
  ...
  // Only sync if editing lesson-database sheet
  if (sheetName !== 'lesson-database') return;
